...
const electronAPI = {
  ...
  onAuthTokens: (callback: (event: any, data: any) => void) => {
    ipcRenderer.on("auth-tokens", callback);
    return () => ipcRenderer.removeListener("auth-tokens", callback);
  },
  // Original methods
  ...
  checkApiKey: () => Promise.resolve(true),
  // validateApiKey: (apiKey: string) => 
  //   ipcRenderer.invoke("validate-api-key", apiKey),
  validateApiKey: (_apiKey: string) => Promise.resolve({ valid: true }),
  ...
  onApiKeyInvalid: (_callback: () => void) => () => {},
